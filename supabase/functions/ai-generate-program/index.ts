...
import { generateProgram, type GenerateProgramInput } from '../_shared/ai-program-generation.ts'

// On-demand only, triggered from the "Générer avec l'IA" form — never on a
...
Deno.serve(async (req) => {
  ...
  let body: {
    ...
    availableExercises?: GenerateProgramInput['availableExercises']
    daysPerWeek?: number
    equipment?: string
    constraints?: string
  }
  ...
  if (!Array.isArray(body.availableExercises) || body.availableExercises.length === 0) {
    return jsonResponse({ error: 'Aucun exercice disponible pour la génération.' }, 400)
  }
  const daysPerWeek = body.daysPerWeek
  if (typeof daysPerWeek !== 'number' || daysPerWeek < 1 || daysPerWeek > 7) {
    return jsonResponse({ error: 'Nombre de jours par semaine invalide.' }, 400)
  }
  ...
  try {
    const proposal = await generateProgram(provider, secretValue, {
