...
- Catalogue : ajout de "Romanian Deadlift" et "Single Leg Press" (groupe `jambes`, cohérent avec "Stiff-Leg Deadlift"/"Single-Leg Romanian Deadlift" déjà présents dans ce groupe). Nom sans "(RDL)" dans les données pour rester cohérent avec le reste du catalogue (aucun autre nom n'a d'abréviation entre parenthèses) — à corriger si le user voulait le sigle littéralement dans le nom.
- Superset : nouvelle colonne `session_template_exercises.superset_group` (texte libre, nullable — ex "A"). Champ optionnel dans `ExerciseSlotFormDialog`, affiché en badge "Superset A" sur `SessionTemplateCard` (vue planification) et `SessionLogPage` (vue log). Portée volontairement limitée à un **tag visuel** pour cette passe — ça ne change pas le flux de saisie (toujours une carte par exercice, une série à la fois). Un flux de saisie alterné (A1 puis A2 puis A1...) serait plus naturel à construire avec le mode gym de P3b plutôt que maintenant.
...
- ⚠️ **Vérification live incomplète** — le user a commencé à utiliser l'app en parallèle pendant cette passe (des exercices réels en français sont apparus sur "Lundi" de FORCE 2026, non touchés). J'ai voulu tester superset + duplication sur "Mardi" (vide) mais l'extension Chrome s'est déconnectée de façon intermittente (3 échecs consécutifs, déjà vu plus tôt dans le projet) — j'ai arrêté plutôt que de m'acharner, conformément à la consigne de ne pas boucler sur des échecs d'outils navigateur. Build/lint/tests unitaires/e2e (8 tests) restent tous verts ; seule la vérification visuelle manuelle de ces deux ajouts reste à faire (par le user directement, ou par moi une fois l'extension stable).
